package si.um.feri.project.soccer;
public class AssetsPaths {


    public static final String GAMEPLAY = "assets/assets/gameplay.atlas";

    public static final String TITLE_FONT = "assets/fonts/thaleah.fnt";
    public static final String FONT = "assets/fonts/pixel2l.fnt";
    public static final String FONT1 = "assets/fonts/pixel.fnt";
    public static final String FONT2 =  "assets/fonts/plump.fnt";
    public static final String GOVERFONT = "assets/fonts/GameOver.fnt";
    public static final String INFO =  "assets/fonts/info.fnt";
    public static final String INFOGREEN =  "assets/fonts/infogreen.fnt";

    public static final String SKIN = "assets/ui/uiskin.json";

    public static final String MUSIC = "assets/sounds/loop.mp3";
    public static final String WHISTLE = "assets/sounds/whistle.mp3";
    public static final String POP= "assets/sounds/pop.mp3";
    public static final String JUMP= "assets/sounds/jump.mp3";
    public static final String CHEER= "assets/sounds/cheer.mp3";
    public static final String COIN= "assets/sounds/coin.mp3";




    private AssetsPaths() {
    }
}
